...
## Architecture: Dual Nature

**Context Tree operates on two complementary levels:**

### 1. Physical Structure: Tree

The **directory hierarchy** provides predictable navigation:
- CLAUDE.md files at each directory level (root, docs/, app/, app/controllers/, etc.)
- Follows source tree structure (brownfield repository organization)
- Enables **scoped context loading** - load root → subdirectory → specific area as needed
- Repository-specific (Context Tree for Project A ≠ Project B)

**Claude Code-first design:** Leverages Claude Code's automatic root CLAUDE.md loading and supports layered context discovery.

### 2. Semantic Relationships: Graph

The **conceptual connections** form a knowledge graph:
- Cross-references between documents at different hierarchy levels
- Terminology maps spanning UI → Code → DB layers
- Architectural patterns referenced from multiple locations
- Single source of truth with many-to-many relationships

**Why both matter:**
- **Tree** enables predictable discovery ("Where's controller context?" → `app/controllers/CLAUDE.md`)
- **Graph** enables conceptual richness (multi-tenancy appears in 5+ places where relevant)
- **Together** they create navigable structure with semantic depth

**Design trade-off:** The name "Context Tree" emphasizes the visible pattern (hierarchical files matching source trees), while the power comes from the graph (rich semantic connections). This is intentional - the tree structure makes the methodology approachable, while graph relationships emerge naturally as you document cross-cutting concerns and verify claims against code.
